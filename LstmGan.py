...
class LstmGan(object):
    ...
    diversity loss
    """
    def get_diversity_loss(self, scores):
        seq_len = len(scores)
        diversity_loss = 0
        for i in range((seq_len - 1)):
            diversity_loss += scores[i] * scores[i + 1]
    ...
    def train(self):
        ...
        #for epoch_i in trange(self.config.n_epochs, desc = 'Epoch'):
            ...
            for batch_i, feature in enumerate(tqdm(self.train_loader,
                ...
                # diversity loss
                diversity_loss = self.get_diversity_loss(scores)
                tqdm.write('diversity_loss: %.3f' % diversity_loss.data[0])
                
                # minimize
                s_e_loss = reconst_loss + sparsity_loss + diversity_loss
    ...
    def test(self):
        ...
        for sample_i, feature in enumerate(tqdm(self.test_loader, desc = 'Evaluate', leave = False)):
            ...
            scores = np.array(scores.data.cpu()).tolist()
